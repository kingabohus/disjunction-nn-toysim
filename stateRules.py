...
def create_training_set(limited, N, prob):
    lengths = np.zeros((N, 1))
    file_name_extension = f"{N}_{int(1/prob)}{'_ltd' if limited else ''}"
    with open(f"./datasets/trainingset_{file_name_extension}.py", 'w') as training_file:
        for n in range(N):
            print(n)
            ...
            [inp, outp] = print_scene(new_scene)
            lengths[n] = len(new_scene.states) - 1
            training_file.write(f"x{n} = ")
            training_file.write(f"{inp}\n")
            training_file.write(f"y{n} = ")
            training_file.write(f"{outp}\n")
        training_file.write(f"training_scenarios{'_ltd' if limited else ''} = [")
        for n in range(N - 1):
            training_file.write(f"(\'example{n}\', x{n}, y{n}), \n")
            # training_file.write('(\'example%d\', x%d, y%d), \n' % (n, n, n))
        training_file.write(f"(\'example{N-1}\', x{N-1}, y{N-1})]")
        # training_file.write('(\'example%d\', x%d, y%d)]' % (N - 1, N - 1, N - 1))
        training_file.write(f"\n# Avg length: {np.mean(lengths)}, N = {N}, p = {prob}")
...
possible_events = [event1, event2, event1, event2, event3, event4, event5, event5, event6, event6, event7, event8, event9, event10,
                   event11, event12, event13, event14, event15, event16]
possible_events_limited = [event1, event2, event1, event2, event3, event4, event5, event5, event6, event6, event7, event8, event9, event10]
...
create_training_set(limited, N, prob)
